		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				if (length(backinter - i.pos) > length(inter - i.pos)) {
					...
					if (i.pos.y < _WorldSpaceCameraPos.y) {
						col = tex2D(_Ground, inter.xz);
					} else {
						col = tex2D(_Ceiling, inter.xz);
					}
				} else {
					col = tex2D(_BackWall, backinter.xy);
				}
				...
				// 0 to 1 float of distance to midnight. 0 at 6pm, 1 at midnight, 0 at 6am
				float timeFromMidnight = clamp((abs(_SimTime - 12) - 6) / 5, 0, 1);
				...
				if (c - _LightingOffset < timeFromMidnight) {
					...
					col.rgb += float3(0.7, 0.6, 0.3); // Yellowy colour
				}
				else {
				...
				float4 windowCol = tex2D(_Window, i.pos.xy);
